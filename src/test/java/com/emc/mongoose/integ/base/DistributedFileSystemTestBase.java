...
extends FileSystemTestBase {
	...
	throws Exception {
		...
		rtConfig.set(BasicConfig.KEY_LOAD_SERVER_ADDRS, ServiceUtil.getHostAddr());
		rtConfig.set(BasicConfig.KEY_RUN_MODE, RUN_MODE_SERVER);
		ServiceUtil.init();
		LOAD_BUILDER_SVC = new MultiLoadBuilderSvc(rtConfig);
		LOAD_BUILDER_SVC.start();
		rtConfig.set(BasicConfig.KEY_RUN_MODE, RUN_MODE_CLIENT);
		rtConfig.set(BasicConfig.KEY_REMOTE_SERVE_JMX, false);
	...
	throws Exception {
		...
		BasicConfig.getContext().set(BasicConfig.KEY_RUN_MODE, RUN_MODE_STANDALONE);
