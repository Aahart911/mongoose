...
implements MutableDataItem {
	...
	public BasicMutableDataItem(final String metaInfo, final ContentSource contentSrc) {
		super(
			metaInfo.substring(0, metaInfo.lastIndexOf(BasicConfig.LIST_SEP)),
			contentSrc
		);
		//
		final String rangesInfo = metaInfo.substring(
			metaInfo.lastIndexOf(BasicConfig.LIST_SEP) + 1, metaInfo.length()
		);
	...
	public synchronized String toString() {
		...
		return strBuilder
			.append(super.toString()).append(BasicConfig.LIST_SEP)
