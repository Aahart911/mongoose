...
implements Runnable {
	...
	//
	public Rampup(final BasicConfig rtConfig) {
	...
	public final void run() {
		...
		for(int i = 0; i < sizeSeq.length; i++) {
			...
			for(final String nextConnCountStr : connCountSeq) {
				...
				rtConfig.set(BasicConfig.KEY_LOAD_LIMIT_COUNT, unChangedCountLimit);
				rtConfig.set(BasicConfig.KEY_DATA_SIZE, nextSizeStr);
				rtConfig.set(BasicConfig.KEY_DATA_SIZE_MIN, nextSizeStr);
				rtConfig.set(BasicConfig.KEY_DATA_SIZE_MAX, nextSizeStr);
				rtConfig.set(BasicConfig.KEY_LOAD_CONNS, nextConnCountStr);
				rtConfig.set(BasicConfig.KEY_APPEND_CONNS, nextConnCountStr);
				rtConfig.set(BasicConfig.KEY_CREATE_CONNS, nextConnCountStr);
				rtConfig.set(BasicConfig.KEY_DELETE_CONNS, nextConnCountStr);
				rtConfig.set(BasicConfig.KEY_READ_CONNS, nextConnCountStr);
				rtConfig.set(BasicConfig.KEY_UPDATE_CONNS, nextConnCountStr);
	...
	public static void main(final String... args) {
		...
		//
		try {
			final Rampup rampupScenario = new Rampup(ctxConfig);
			rampupScenario.run();
